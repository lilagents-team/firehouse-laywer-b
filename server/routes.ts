...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Helper function to load newsletters (with caching)
  async function loadNewsletters(): Promise<CompressedNewsletter[]> {
    try {
      ...
      try {
        ...
        useCache = cacheAge < 5 * 60 * 1000;
      } catch {
        // Cache doesn't exist
        useCache = false;
      }
      
      // Generate index if cache is stale or doesn't exist
      if (!useCache) {
      ...
      // Fallback to existing newsletter index
      try {
